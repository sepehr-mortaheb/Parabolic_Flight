...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None, deg=degree):
    ...
    if 'frameRate' in expInfo and expInfo['frameRate'] is not None:
        ...
        frameDur = 1.0 / 60.06  # could not measure, so guess (REFRESH RATE)
    ...
    Vertical = visual.ImageStim(
        win=win,
        name='Vertical', 
        image=f'./images2/circle-grey.png', mask=None, anchor='center',
    ...
    df.to_excel('./loopTemplate270.xlsx', index=False)
    #---------------------------------------------------------------------------
    ...
    for thisTrial in trials:
        ...
        # --- Run Routine "VDT" ---
        routineForceEnded = not continueRoutine
        jitter = 0.1 #seconds (RANDOMIZED INTER TRIAL INTERVAL) 
