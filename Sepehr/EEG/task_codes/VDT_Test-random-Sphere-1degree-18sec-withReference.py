...
expName = 'VDT_Test-random-Sphere-1degree-18sec-withReference'  # from the Builder filename that created this script
expInfo = {
    'participant': "",
    'session': paradigm,
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None, deg=degree):
    ...
    if paradigm == 'main':
        ...
        for i in range(len(df)):
            if arr3[i] == 1:
                df.loc[i, 'image'] = f'Sphere-CW-{deg}.png'
            elif arr3[i] == 2:
                df.loc[i, 'image'] = f'Sphere-CCW-{deg}.png'

    elif paradigm == 'control1':
        df['image'] = ['Sphere-Yellow.png']*len(arr3)
        for i in range(len(df)):
            if arr3[i] == 1:
                df.loc[i, 'image'] = f'Sphere-CW-{deg}.png'
            elif arr3[i] == 2:
        ...
        for i in range(len(df)):
            if arr3[i] == 1:
                if arr3_c[i]:
                    ...
                    df.loc[i, 'image'] = f'Sphere-Yellow-CW{deg}.png'
            elif arr3[i] == 2:
                if arr3_c[i]:
                    df.loc[i, 'image'] = f'Sphere-CCW-{deg}.png'
                else:
                    df.loc[i, 'image'] = f'Sphere-Yellow-CCW{deg}.png'
            elif arr3[i] == 0:
                if arr3_c[i]:
                    df.loc[i, 'image'] = f'Sphere-White.png'
                else:
                    df.loc[i, 'image'] = f'Sphere-Yellow.png'
